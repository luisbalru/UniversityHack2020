...
def plotData(X, y, route):
    ...
    plt.scatter(cl3[:,0], cl3[:,1], color = colors[2], label = "RETAIL")
    plt.scatter(cl4[:,0], cl4[:,1], color = colors[2], label = "OFFICE")
    plt.scatter(cl5[:,0], cl5[:,1], color = colors[2], label = "OTHER")
    plt.scatter(cl6[:,0], cl6[:,1], color = colors[2], label = "AGRICULTURE")
...
resultado.to_csv('UGR_Data R Us.txt',sep='|',index=False)
